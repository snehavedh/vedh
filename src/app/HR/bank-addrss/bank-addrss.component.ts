...
@Component({
  selector: 'app-bank-addrss',
  templateUrl: './bank-addrss.component.html',
  styleUrls: ['./bank-addrss.component.sass']
})
export class BankAddrssComponent implements OnInit {
...
get f() { return this.ReasonForm.controls; }
  ...
  get_Item(item:any){
    ...
    // console.log(item)
    this.empData = item;
    let param = {"userName":""+this.empData.EMPLOYEEID+"", "REQUESTTYPE":"BANKADD"}
    this.authService.empProfileEditReqsts(param).subscribe(res=>{
      ...
      this.myArray = {"EMPLOYEEID":""+this.empData.EMPLOYEEID+"","REQUESTTYPE":"BANKADD", "ACTIONTYPE": "A", "ACTION_BY": ""+this.userData.user.empID+"" };
