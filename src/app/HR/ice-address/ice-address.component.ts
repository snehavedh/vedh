...
@Component({
  selector: 'app-ice-address',
  templateUrl: './ice-address.component.html',
  styleUrls: ['./ice-address.component.sass']
})
export class IceAddressComponent implements OnInit {
  myDate:any;
  veiwModal:any;
  listOfReqsts:any;
  empData:any;
...
getApiData(){  
  this.isLoading = true;
  let editBlockParam= {"REQUESTTYPE": "ICEADD", "empID": "" + this.userData.user.empID + ""}
...
get f() { return this.ReasonForm.controls; }
  ...
  get_Item(item:any){
    ...
    // console.log(item)
    this.empData = item;
    let param = {"userName":""+this.empData.EMPLOYEEID+"", "REQUESTTYPE":"ICEADD"}
    this.authService.empProfileEditReqsts(param).subscribe(res=>{
      ...
      this.myArray = {"EMPLOYEEID":""+this.empData.EMPLOYEEID+"","REQUESTTYPE":"ICEADD", "ACTIONTYPE": "A", "ACTION_BY": ""+this.userData.user.empID+"" };
