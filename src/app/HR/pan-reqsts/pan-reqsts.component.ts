...
@Component({
  selector: 'app-pan-reqsts',
  templateUrl: './pan-reqsts.component.html',
  styleUrls: ['./pan-reqsts.component.sass']
})
export class PanReqstsComponent implements OnInit {
...
getApiData(){
  ...
  let editBlockParam= {"REQUESTTYPE": "PANADD", "empID": "" + this.userData.user.empID + ""}
  // console.log(editBlockParam)
...
get f() { return this.ReasonForm.controls; }
  ...
  get_Item(item:any){
    ...
    // console.log(item)
    this.empData = item;
    let param = {"userName": ""+this.empData.EMPLOYEEID+"", "REQUESTTYPE":"PANADD"}
    this.authService.empProfileEditReqsts(param).subscribe(res=>{
      ...
      this.myArray = {"EMPLOYEEID":""+this.empData.EMPLOYEEID+"","REQUESTTYPE":"PANADD", "ACTIONTYPE": "A", "ACTION_BY": ""+this.userData.user.empID+""};
      for(let i = 0; i<this.reqData.length; i ++){
        ...
        if(this.reqData[i].FIELD_DISPLAY.split('.')[1]=='pdf'){
          let PDF_filepath = this.ImgBasepath + this.reqData[i].FIELD_DISPLAY+ "#toolbar=0"; 
          // this.pdfData = this.dom.bypassSecurityTrustResourceUrl(PDF_filepath); 
          this.pdfData = PDF_filepath; 
