...
@Component({
  selector: 'app-permnt-addrss',
  templateUrl: './permnt-addrss.component.html',
  styleUrls: ['./permnt-addrss.component.sass']
})
export class PermntAddrssComponent implements OnInit {
...
getApiData(){
  this.isLoading = true;
  let editBlockParam= {"REQUESTTYPE": "PERADD", "empID": "" + this.userData.user.empID + ""}
...
get f() { return this.ReasonForm.controls; }
  ...
  get_Item(item:any){
    ...
    // console.log(item)
    this.empData = item;
    let param = {"userName":""+this.empData.EMPLOYEEID+"", "REQUESTTYPE":"PERADD"}
    this.authService.empProfileEditReqsts(param).subscribe(res=>{
      ...
      this.myArray = {"EMPLOYEEID":""+this.empData.EMPLOYEEID+"","REQUESTTYPE":"PERADD", "ACTIONTYPE": "A","ACTION_BY": ""+this.userData.user.empID+""};
...
approve(){
  ...
  // let param = this.myArray;
  // let x= { "APPROVED_BY": ""+this.userData.user.empID+""}
  // let y= Object.assign(param,x );
  // let finalParam =  JSON.stringify(y);
