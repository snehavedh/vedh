...
@Component({
  selector: 'app-assessment-permanent-report',
  templateUrl: './assessment-permanent-report.component.html',
  styleUrls: ['./assessment-permanent-report.component.sass']
})
export class AssessmentPermanentReportComponent implements OnInit {
  loggedUser: any = {};
  userData: any;
  myDate:any;
  empObj:any;
  ...
  authBoolean:boolean
  privil_eges:any= {}; 
  fileName= 'Assessment_Permanent_Report.xlsx';
...
// End of ngOnInit 
  ...
  getapiData(){   
    ...
    for (let i = 0; i < this.privil_eges.length; i++) {   
      if(this.privil_eges[i].Assessment_Permant_Report == "true"){ 
  ...
  this.isLoading = true;   
  this.authService.assesment_permanentreport(this.empObj).subscribe(
    (res)=>{   
      this.isLoading = false;
      // console.log(res.permanentreport)
      this.assmntData = res.permanentreport; 
