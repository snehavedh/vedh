...
@Component({
  selector: 'app-flexi-policy',
  templateUrl: './flexi-policy.component.html',
  styleUrls: ['./flexi-policy.component.sass']
})
export class FlexiPolicyComponent implements OnInit {
  loggedUser: any = {};
  userData: any;
  myDate:any;
  empObj:any;
  ...
  getApiData(){ 
    ...
    for (let i = 0; i < this.privil_eges.length; i++) {   
      if(this.privil_eges[i].Flexi_Policy == "true"){ 
  ...
  getList(){
    ...
    if(this.flexiPolcyForm.invalid){  
      ...
      // console.log(this.flexiPolcyForm.value)
      this.authService.empFlexiGetList(this.flexiPolcyForm.value).subscribe(res=>{
        // console.log(res)
        this.isLoading =false;
...
numberOnly(event): boolean {
  ...
  flxyAction(item:any, event:any){ 
    ...
    this.authService.empFlexiAssignRemove(params).subscribe(res=>{
      // console.log(res)
      if(res.count == '1'){
        if(item.ADD =='ADD'){
          item.ADD = 'ASSIGNED';
